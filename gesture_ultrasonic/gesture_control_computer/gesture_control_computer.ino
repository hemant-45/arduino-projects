...
const int trigPin1 = 11;  // the number of the trigger output pin ( sensor 2 ) 
const int echoPin1 = 10;  // the number of the echo input pin ( sensor 2 ) 
...
void loop()
{
  ...
  if(distance<=35 && distance>=15) // once if we placed our hands in front of the right sensor in the range between 15 to 35cm this condition becomes true.
  { 
          // this loop measures the distance for another 300 milliseconds. ( it helps to find the difference between the swipe and stay of our hand in front of the right sensor )
    find_distance();
    if(distance<=35 && distance>=15) // this condition will true if we place our hand in front of the right sensor for more then 300 milli seconds. 
    {
     temp=distance;                         // store the current position of our hand in the variable temp. 
     while(distance<=35 || distance==50)    // this loop will run untill we removes our hand in front of the right sensor.
     {
       find_distance();                      // call this function continuously to get the live data. 
       if((temp+6)<distance)                // this condition becomes true if we moves our hand away from the right sensor (**but in front of it ). here " temp+6 " is for calibration.
       {
       Serial.println("down");               // send "down" serially.
       }
       else if((temp-6)>distance)           // this condition becomes true if we moves our hand closer to the right sensor.
       {
        Serial.println("up");                // send "up" serially.
       }
     }
                                     // this condition becomes true, if we only swipe in front of the right sensor . 
